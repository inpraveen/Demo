...
class signInVC: UIViewController {
    ...
    @IBOutlet weak var companyIDTxt: UITextField!
...
//        signUpBtn.frame = CGRect(x: self.view.frame.size.width - self.view.frame.size.width / 4 - 20, y: signInBtn.frame.origin.y, width: self.view.frame.size.width / 4, height: 30)
  ...
  @objc  func hideKeyboard(_ recognizer : UITapGestureRecognizer) {
    ...
    @IBAction func signInBtn_click(_ sender: AnyObject) {
        ...
        if usernameTxt.text!.isEmpty || passwordTxt.text!.isEmpty || companyIDTxt.text!.isEmpty || urlTxt.text!.isEmpty {
...
//            } else {
        ...
        PFUser.logInWithUsername(inBackground: usernameTxt.text!, password: passwordTxt.text!, companyID: companyIDTxt.text! , url: urlTxt.text! ){(user, error) -> Void in
